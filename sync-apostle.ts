...
async function syncApostle() {
  const cat = CATEGORIES.find(c => c.title === "Apostle's Creed");
  if (!cat) {
    console.error("Apostle's Creed not found");
    return;
  }
  ...
  console.log("Successfully synced Apostle's Creed to Firestore");
}

syncApostle().catch(console.error);
