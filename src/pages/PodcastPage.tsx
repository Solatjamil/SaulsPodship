      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-8">
        ...
        <div className="space-y-6">
          {EPISODES.map((ep) => (
            <article
              key={ep.id}
              className="relative cursor-pointer p-6 sm:p-8 rounded-3xl bg-white border border-gray-200 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row md:items-center justify-between gap-6"
            >
              <Link to={`/podcast/${ep.slug}`} aria-hidden="true" tabIndex={-1} className="absolute inset-0 z-[1] rounded-3xl hover:shadow-md" />
              ...
              <div className="flex flex-row md:flex-col items-center gap-3 flex-shrink-0">
                <Link
                  to={`/podcast/${ep.slug}`}
                  className="relative z-[2] w-full md:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-[#4A152C] text-[#E8C96A] font-bold text-xs shadow hover:bg-[#681E3E] transition-all"
                >
                ...
                {ep.youtubeUrl && (
                  <a
                    ...
                    className="relative z-[2] w-full md:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-red-600 text-white font-bold text-xs shadow hover:bg-red-500 transition-all"
                  >
