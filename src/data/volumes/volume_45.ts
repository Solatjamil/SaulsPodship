/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 */

import { Volume } from '../../../types';

export const volume_45: Volume = {
  "id": "45",
  "number": 45,
  "slug": "the-magi",
  "title": "The Magi",
  "subtitle": "Persian Astrologers, the Bethlehem Star & Royal Epiphany",
  "overview": "Separating biblical truth from legend: Who were the Magi? Persian Zoroastrian astrologers, Daniel's prophecy, the Star of Bethlehem, and the meaning of gold, frankincense, and myrrh.",
  "category": "history",
  "lastUpdated": "2026-09-06",
  "heroImage": {
    "src": "/images/volumes/the-magi/hero-1920.jpg",
    "alt": "Public-domain artwork illustrating “The Magi”",
    "credit": "Public domain — Edward Burne-Jones · Wikimedia Commons"
  },
  "cardImage": {
    "src": "https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?auto=format&fit=crop&w=800&q=80",
    "alt": "The Magi"
  },
  "metaDescription": "Who were the biblical Magi (Wise Men)? Trace their Persian origins, Daniel's influence in Babylon, the astronomical Star of Bethlehem, and royal prophetic gifts.",
  "keywords": [
    "the magi wise men",
    "star of bethlehem astronomy",
    "gold frankincense myrrh meaning",
    "persian magi zoroastrianism",
    "matthew 2 wise men history",
    "how many magi visited jesus"
  ],
  "articleLink": "https://goshsays.blogspot.com/2026/01/the-magi-complete-historical-and.html",
  "youtubeLink": "https://youtu.be/Wn_xVTslokI",
  "youtubeStatus": "scheduled",
  "youtubePublishDate": "2027-02-17",
  "relatedVolumes": [
    "christmas-history",
    "messianic-prophecies",
    "family-tree-adam-jesus",
    "jesus-is-god-full-references-ot-nt"
  ],
  "summary": "The Magi investigates the identity, astronomical knowledge, and theological journey of the mysterious travelers who visited the young child Jesus in Matthew 2. It demonstrates how Daniel's legacy among Babylonian and Persian astronomers preserved messianic hope that culminated in the epiphany of Christ.",
  "keyFacts": [
    "Scripture never specifies that there were three Magi; the tradition of three arose in early church history due to the three recorded gifts (gold, frankincense, myrrh).",
    "The Magi were hereditary Persian/Parthian priestly astronomers and royal kingmakers, not reigning monarchs.",
    "Daniel had been appointed chief over the Babylonian magi (Daniel 2:48), likely transmitting Hebrew prophecies of a coming Judean messianic king.",
    "The gifts prophesied Christ's three offices: Gold for His Kingship, Frankincense for His Deity and Priesthood, and Myrrh for His sacrificial burial."
  ],
  "faq": [
    {
      "q": "Were the Magi present at the stable when Jesus was born?",
      "a": "No; Matthew 2:11 notes they visited Jesus as a young 'child' living in a 'house' in Bethlehem, likely months or up to two years after His birth (which prompted Herod to kill children up to two years old)."
    },
    {
      "q": "What was the Star of Bethlehem?",
      "a": "Scholars suggest it was a miraculous celestial light, an astronomical planetary conjunction (such as Jupiter and Saturn in 7 BC), or a supernova that guided the Magi southwest toward Judea."
    },
    {
      "q": "How did Persian astrologers know to seek a Jewish King?",
      "a": "Centuries earlier, the prophet Daniel was chief of the royal sages in Babylon and Persia; he shared Balaam's prophecy ('a star shall come out of Jacob,' Num 24:17) and Daniel 9's 70-weeks timeline."
    },
    {
      "q": "What is the theological significance of the Magi's visit?",
      "a": "The Magi represent the first Gentile worshippers bowing before the King of the Jews, fulfilling prophecies that nations would come to the brightness of Israel's rising (Isaiah 60:3)."
    }
  ],
  "content": {
    "analysis": "### **I. Who Were the Magi?**\nThe Magi (Greek: **μάγοι**, *magoi*) were a priestly caste from the ancient East, likely Persia (modern Iran) or Babylon (modern Iraq). In antiquity, they were the \"scientists\" and \"philosophers\" of their day. They were known for:\n• **Astronomy & Astrology**: Mapping the stars to understand divine decrees.\n• **Dream Interpretation**: Acting as bridge-builders between the spiritual and physical realms.\n• **Political Counsel to Kings**: They were high-level advisors and \"king-makers.\"\n• **Studying Ancient Prophecies**: They kept records of various nations' sacred texts.\n\nThey were not kings themselves, but held significant political influence. The Prophet **Daniel** was appointed \"chief of the magicians, enchanters, astrologers and diviners\" (Magi) in Babylon (**Daniel 2:48**).\n\n---\n\n### **II. The Daniel Connection**\nThe most likely reason Persian Magi were searching for a \"King of the Jews\" is the influence of Daniel. While in exile, Daniel became the head of the Babylonian/Persian Magian order. He likely shared the prophecy of the **Seventy Weeks** (**Daniel 9**), which provided a specific timeline for the arrival of the \"Anointed One, the ruler.\" The Magi of Jesus' day were likely the intellectual descendants of Daniel's students.\n\n---\n\n### **III. The Star of Bethlehem**\nThe Magi followed a \"star\" (*aster* in Greek). Biblical scholars and astronomers have debated its nature:\n• **Natural Explanation**: A rare conjunction of Jupiter and Saturn, or a supernova.\n• **Supernatural Explanation**: The **Shekinah Glory** of God—the same pillar of cloud and fire that led Israel in the Exodus—appearing as a localized celestial light to guide the Magi to the exact house.\n\n---\n\n### **IV. The Three Gifts and Their Meaning**\nThe Bible does not state there were exactly three Magi, but tradition infers this from the three types of gifts offered, which carry deep theological weight:\n1. **Gold**: A gift for a **King**. Recognizing Jesus' royal sovereignty and right to rule.\n2. **Frankincense**: A gift for a **Priest**. An incense used in temple worship, recognizing His role as mediator.\n3. **Myrrh**: A gift for a **Sacrifice**. An embalming spice, prophetically pointing to His future death and burial for the sins of the world.\n\n---\n\n### **V. The Significance of the Visit**\nThe visit of the Magi represents the **Gentile Epiphany**. It demonstrates that Jesus is not just the Messiah of Israel, but the Light of the World, sought and worshiped by the ends of the earth.",
    "tables": [
      {
        "title": "TRADITION VS. SCRIPTURE",
        "headers": [
          "Topic",
          "Popular Tradition",
          "Biblical Record"
        ],
        "rows": [
          [
            "Number",
            "Exactly Three",
            "Unspecified (likely a large caravan)"
          ],
          [
            "Names",
            "Caspar, Melchior, Balthazar",
            "Anonymous"
          ],
          [
            "Timing",
            "At the stable on Christmas night",
            "Months or years later in a house"
          ],
          [
            "Status",
            "Three Kings",
            "Wise Men / Magi (Priestly Class)"
          ],
          [
            "Ages",
            "Representing three stages of life",
            "Not mentioned"
          ]
        ]
      },
      {
        "title": "THE PROPHETIC GIFTS",
        "headers": [
          "Gift",
          "Symbolism",
          "Sacred Office"
        ],
        "rows": [
          [
            "Gold",
            "Purity, Value, Royalty",
            "Kingship (Son of David)"
          ],
          [
            "Frankincense",
            "Prayer, Worship, Divinity",
            "Priesthood (High Priest)"
          ],
          [
            "Myrrh",
            "Bitterness, Suffering, Embalming",
            "Savior (The Atonement)"
          ]
        ]
      }
    ],
    "storyPanels": [
      {
        "id": "magi-1",
        "title": "The Great Observation",
        "description": "Magi in an ancient Persian observatory, surrounded by celestial maps and the scrolls of Daniel, witnessing the rising of the Star.",
        "scripture": "Matthew 2:2",
        "imagePrompt": "Ornately dressed Persian scholars on a stone observatory roof at night, glowing astrolabes, ancient Hebrew scrolls open on tables, a brilliant white-blue star reflecting in their eyes, epic atmosphere.",
        "colorTheme": "#1D2D50",
        "era": "Christ"
      },
      {
        "id": "magi-2",
        "title": "The Long Journey",
        "description": "The caravan of the Magi crossing the vast Arabian desert, guided by the singular radiant light in the night sky.",
        "scripture": "Matthew 2:1",
        "imagePrompt": "A massive caravan of camels with rich silk canopies moving across golden sand dunes under a giant silver moon and a radiant guiding star, cinematic scale.",
        "colorTheme": "#8B1E3F",
        "era": "Christ"
      },
      {
        "id": "magi-3",
        "title": "The Royal Adoration",
        "description": "The Magi bowing in a simple Judean home, opening their treasures before the young Child.",
        "scripture": "Matthew 2:11",
        "imagePrompt": "Inside a simple stone house, the Magi in vibrant robes kneeling on the floor, holding out golden boxes, a young child on Mary's lap bathed in warm golden light, emotional and sacred atmosphere.",
        "colorTheme": "#D4AF37",
        "era": "Christ"
      }
    ],
    "timeline": [],
    "scriptures": [],
    "interactiveMapPoints": [],
    "characterProfiles": [],
    "maps": []
  },
  "featured": false
};

export default volume_45;
