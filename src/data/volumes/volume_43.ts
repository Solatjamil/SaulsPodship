/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 */

import { Volume } from '../../../types';

export const volume_43: Volume = {
  "id": "43",
  "number": 43,
  "slug": "ten-commandments-ot-laws",
  "title": "Ten Commandments & OT Laws",
  "subtitle": "The Decalogue, Mosaic Covenant & the Law in the Christian Life",
  "overview": "The Ten Commandments given at Sinai: their historical covenant structure, moral permanence, tripartite law classification (Moral, Civil, Ceremonial), and role in Christ.",
  "category": "scholarly",
  "lastUpdated": "2026-09-06",
  "heroImage": {
    "src": "/images/volumes/ten-commandments-ot-laws/hero-1920.jpg",
    "alt": "Public-domain artwork illustrating “Ten Commandments & OT Laws”",
    "credit": "Public domain — Gustave Doré · Wikimedia Commons"
  },
  "cardImage": {
    "src": "https://images.unsplash.com/photo-1447069387593-a5de0862481e?auto=format&fit=crop&w=800&q=80",
    "alt": "Ten Commandments & OT Laws"
  },
  "metaDescription": "In-depth study of the Ten Commandments and Old Testament Law: Decalogue structure, the threefold division of the Law, Christ fulfilling the Law, and modern Christian ethics.",
  "keywords": [
    "ten commandments meaning",
    "old testament law for christians",
    "moral civil ceremonial law",
    "decalogue commentary",
    "has christ fulfilled the law",
    "mosaic covenant sinai"
  ],
  "articleLink": "https://goshsays.blogspot.com/2026/01/the-ten-commandments-and-old-testament.html",
  "youtubeLink": "https://youtu.be/K-8k1Pq0Y_U",
  "youtubeStatus": "scheduled",
  "youtubePublishDate": "2027-02-10",
  "relatedVolumes": [
    "the-five-offerings-of-leviticus",
    "sermon-on-the-mount",
    "guide-to-christian-living",
    "all-bible-stories"
  ],
  "summary": "Ten Commandments & OT Laws investigates the covenantal law delivered by Yahweh to Israel at Mount Sinai. It examines the Decalogue's suzerain-vassal treaty form, the historic threefold division into moral, civil, and ceremonial laws, and how Christ fulfills the Law to write it upon believers' hearts.",
  "keyFacts": [
    "The Ten Commandments (Decalogue) form the moral bedrock of the Sinai covenant, divided into duty toward God (Commands 1–4) and duty toward neighbor (Commands 5–10).",
    "Historic theology categorizes the Mosaic Law into Moral (perpetual reflection of God's character), Ceremonial (sacrifices pointing to Christ), and Civil (Israel's national civic code).",
    "Jesus summarized the entire Law in two Great Commandments: loving God with all our heart, soul, and mind, and loving our neighbor as ourselves (Matthew 22:37-40).",
    "The Three Uses of the Law (Usus Legis) serve as a mirror revealing sin (pedagogical), a bridle restraining civic evil, and a lamp guiding Christian sanctification."
  ],
  "faq": [
    {
      "q": "Are Christians still obligated to keep the Ten Commandments?",
      "a": "Yes; while believers are not under the Law as a covenant of works to earn salvation, the moral law permanently reflects God's holy character and serves as a loving guide for Christian sanctification."
    },
    {
      "q": "Why don't Christians observe Old Testament food laws or animal sacrifices?",
      "a": "The Ceremonial laws were dietary and sacrificial shadows that found their complete, once-for-all fulfillment in the finished work and pure sacrifice of Jesus Christ (Hebrews 10:1-10, Mark 7:19)."
    },
    {
      "q": "Why do Christians worship on Sunday rather than Saturday (Sabbath)?",
      "a": "The early Church gathered on Sunday ('the Lord's Day') to celebrate the resurrection of Jesus Christ, which inaugurated the new creation and true spiritual Sabbath rest (Hebrews 4:9-10)."
    },
    {
      "q": "What does Paul mean that 'Christ is the end of the law' (Romans 10:4)?",
      "a": "The Greek telos means 'goal' or 'culmination'—Christ is the ultimate aim toward which the entire Law pointed, offering righteousness to everyone who believes."
    }
  ],
  "content": {
    "analysis": "### **I. The Structure of the Old Testament Law**\nThe Old Testament contains **613 commandments** (Mitzvot) in Jewish tradition. Theologians traditionally organize them into three distinct categories to understand their application today:\n\n1. **Moral Law**: Eternal and unchanging. Rooted in God’s character and summarized in the **Ten Commandments**.\n2. **Civil (Judicial) Law**: Laws for Israel as a nation-state (justice, property, penalties). Underlying principles reflect God's justice but the specific penalties were for the theocracy.\n3. **Ceremonial Law**: Rituals, sacrifices, and festivals. Designed to point to Christ and fulfilled in His high priestly work.\n\n---\n\n### **II. The Moral Law (Eternal)**\nThe Moral Law is the only part of the OT Law that remains strictly binding in its letter and spirit for all people at all times.\n• **Rooted in Character**: It reflects God's holiness.\n• **Reaffirmed by Jesus**: In the Sermon on the Mount, Jesus deepened the law, showing that it applies to the heart, not just outward actions (**Matthew 5–7**).\n• **Summary**: Love for God (Commands 1–4) and Love for Neighbor (Commands 5–10).\n\n---\n\n### **III. The Civil Law (Justice Principles)**\nWhile the specific punishments (like stoning) belonged to the ancient Israelite context, the **principles** remain vital for modern ethics:\n• **Human Dignity**: Protection of the weak and marginalized.\n• **Equity**: Fair weights and honest business practices.\n• **Accountability**: Restitution for theft or negligence.\n\n---\n\n### **IV. The Ceremonial Law (Fulfilled in Christ)**\nJesus fulfilled the ceremonial system by becoming:\n• **The True Temple**: The dwelling place of God (**John 2:19**).\n• **The Final Sacrifice**: The Lamb who takes away the sin of the world (**Hebrews 10:1–14**).\n• **The High Priest**: Our eternal mediator (**Hebrews 4:14–16**).\n\n---\n\n### **VII. Jesus & the Law — Fulfillment, Not Abolition**\nJesus stated: *“I have not come to abolish the Law but to fulfill it”* (**Matthew 5:17**). \nFulfillment means He completed the sacrificial system, showed perfect obedience, and revealed the true \"heart-intent\" of the commandments. Under the New Covenant, the Spirit writes these laws on our hearts (**Jeremiah 31:33**).\n\n---\n\n### **X. How Christians Use the OT Law Today**\n• **Mirror**: To reveal our sin and need for a Savior.\n• **Restraint**: To provide a baseline for civil order in society.\n• **Guide**: To show believers the kind of life that pleases God.",
    "tables": [
      {
        "title": "V. THE TEN COMMANDMENTS (DECALOGUE)",
        "headers": [
          "#",
          "Commandment",
          "NT Confirmation",
          "Meaning in Daily Life"
        ],
        "rows": [
          [
            "1",
            "No other gods",
            "Matthew 4:10",
            "God alone deserves ultimate loyalty."
          ],
          [
            "2",
            "No idols",
            "1 John 5:21",
            "Reject material or emotional idols."
          ],
          [
            "3",
            "No misusing God’s name",
            "Matthew 6:9",
            "Honor God in speech and life."
          ],
          [
            "4",
            "Keep Sabbath holy",
            "Hebrews 4:9–11",
            "Rest, worship, and trust God's provision."
          ],
          [
            "5",
            "Honor parents",
            "Matthew 15:4",
            "Respect authority and household order."
          ],
          [
            "6",
            "Do not murder",
            "Matthew 5:21–22",
            "Guard life; avoid hatred and violence."
          ],
          [
            "7",
            "Do not commit adultery",
            "Matthew 5:27–28",
            "Purity in thought and body."
          ],
          [
            "8",
            "Do not steal",
            "Ephesians 4:28",
            "Respect property; be generous."
          ],
          [
            "9",
            "No false witness",
            "Ephesians 4:25",
            "Be truthful; reject gossip."
          ],
          [
            "10",
            "Do not covet",
            "Luke 12:15",
            "Gratitude; avoid envy and greed."
          ]
        ]
      },
      {
        "title": "VI. NUMBERING DIFFERENCES",
        "headers": [
          "Tradition",
          "How They Divide Commandments"
        ],
        "rows": [
          [
            "Jewish",
            "Commandment 1 is \"I am the LORD your God...\""
          ],
          [
            "Catholic / Lutheran",
            "Combine 1 & 2; split 10 (coveting wife vs. property)."
          ],
          [
            "Protestant / Orthodox",
            "Traditional 10-separations (as listed above)."
          ]
        ]
      },
      {
        "title": "VIII. LAW VS. GRACE SUMMARY",
        "headers": [
          "Law",
          "Grace"
        ],
        "rows": [
          [
            "Reveals sin",
            "Removes sin"
          ],
          [
            "Shows holiness",
            "Empowers holiness"
          ],
          [
            "Written on stone",
            "Written on hearts"
          ],
          [
            "Points to Christ",
            "Fulfilled in Christ"
          ],
          [
            "Condemns sinner",
            "Saves sinner"
          ]
        ]
      }
    ],
    "storyPanels": [
      {
        "id": "law-1",
        "title": "The Thunder of Sinai",
        "description": "Moses ascending the mountain shrouded in smoke and fire to receive the tablets of the testimony.",
        "scripture": "Exodus 20:18",
        "imagePrompt": "A massive dark mountain peak engulfed in thick smoke and lightning, a tiny figure of a man in robes ascending toward a glowing summit, epic cinematic scale.",
        "colorTheme": "#1D2D50",
        "era": "Exodus"
      },
      {
        "id": "law-2",
        "title": "The Heart of the Law",
        "description": "Jesus teaching the true depth of the commandments during the Sermon on the Mount.",
        "scripture": "Matthew 5:17",
        "imagePrompt": "Jesus seated on a green hillside, sunlight filtering through trees, a diverse group of listeners looking on in awe, peaceful and profound atmosphere.",
        "colorTheme": "#4CAF50",
        "era": "Christ"
      }
    ],
    "timeline": [],
    "scriptures": [],
    "interactiveMapPoints": [],
    "characterProfiles": [],
    "maps": []
  },
  "featured": false
};

export default volume_43;
