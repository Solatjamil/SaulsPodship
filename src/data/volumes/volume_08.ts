/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 */

import { Volume } from '../../../types';

export const volume_08: Volume = {
  "id": "08",
  "number": 8,
  "slug": "baptism-sacraments-circumcision",
  "title": "Baptism, Sacraments & Circumcision",
  "subtitle": "Signs and Seals of the Old and New Covenants",
  "overview": "Tracing the theological transition from Abrahamic circumcision to Christian water baptism, analyzing sacramental theology across church traditions.",
  "category": "scholarly",
  "lastUpdated": "2026-09-06",
  "heroImage": {
    "src": "/images/volumes/baptism-sacraments-circumcision/hero-1920.jpg",
    "alt": "Public-domain artwork illustrating “Baptism, Sacraments & Circumcision”",
    "credit": "Public domain — St1hart · Wikimedia Commons"
  },
  "cardImage": {
    "src": "https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=800&q=80",
    "alt": "Baptism, Sacraments & Circumcision"
  },
  "metaDescription": "Understand Christian baptism, Old Testament circumcision, and sacraments. Explore sacramental theology, covenant signs, immersion vs pouring, and grace.",
  "keywords": [
    "christian baptism",
    "sacraments theology",
    "circumcision to baptism",
    "covenant signs",
    "believers baptism vs infant",
    "water baptism meaning"
  ],
  "articleLink": "https://goshsays.blogspot.com/2026/01/baptism-sacraments-and-circumcision.html",
  "youtubeLink": "https://youtu.be/taqcHYihXxY",
  "youtubeStatus": "scheduled",
  "youtubePublishDate": "2026-10-07",
  "relatedVolumes": [
    "holy-communion",
    "all-big-denominations-their-differences",
    "guide-to-christian-living",
    "the-five-offerings-of-leviticus"
  ],
  "summary": "Baptism, Sacraments & Circumcision examines the external signs and seals established by God to mark His covenant people. It traces how Old Testament circumcision of the flesh foreshadowed the New Testament spiritual circumcision of the heart signified by water baptism.",
  "keyFacts": [
    "Circumcision was instituted in Genesis 17 as the physical sign of God's everlasting covenant with Abraham and his descendants.",
    "Paul connects baptism directly to circumcision in Colossians 2:11-12, calling it the 'circumcision made without hands.'",
    "Christian baptism signifies union with Christ in His death, burial, and resurrection (Romans 6:3-4).",
    "Historic theological camps differ on whether sacraments operate as efficacious means of grace (Sacramentalism) or symbolic ordinances (Memorialism)."
  ],
  "faq": [
    {
      "q": "How does baptism relate to Old Testament circumcision?",
      "a": "Both serve as covenant signs marking belonging to God's people; circumcision marked physical descent in Israel, while baptism marks spiritual rebirth and incorporation into the Body of Christ."
    },
    {
      "q": "What is the scriptural mode of baptism?",
      "a": "The Greek word baptizo signifies immersion, which vividly pictures going down into the grave with Christ and rising to new life, though some traditions practice pouring or sprinkling."
    },
    {
      "q": "Does baptism save you?",
      "a": "Orthodox Protestant theology teaches that salvation is by grace through faith alone; baptism is the outward sign and seal of inward regenerative grace, not a magical ritual."
    },
    {
      "q": "Why was Jesus baptized if He had no sin?",
      "a": "Jesus was baptized to 'fulfill all righteousness' (Matthew 3:15), identifying with repentant humanity, inaugurating His public messianic ministry, and validating John's baptism."
    }
  ],
  "content": {
    "analysis": "### **Baptism — Meaning, Power, Purpose**\nBaptism is the New Testament entrance sign into God’s covenant family. It serves as the bridge between personal faith and corporate belonging, representing a profound ontological shift in the believer's standing before the Creator.\n\nIt represents:\n• **Dying with Christ**: The radical end of the old life and the power of sin.\n• **Resurrection**: Being raised by the Spirit to walk in permanent newness of life.\n• **Washing**: The symbolic removal of spiritual filth through the blood of the Lamb.\n• **Identity**: A public, non-reversible declaration of belonging to the Triune God.\n\n*Key Verses: Matthew 3:13–17; Acts 2:38; Romans 6:3–4; Colossians 2:12; 1 Peter 3:21*\n\n---\n\n### **Sacraments — Visible Words**\nTheologians often describe a sacrament as a \"Visible Word\"—a physical sermon that we taste, touch, or feel.\n\n*“An outward and visible sign of an inward and spiritual grace, instituted by Christ himself.”*\n\nThese are not merely empty rituals or social traditions. They are covenant markers through which God, by His Spirit, strengthens the faith of the recipient and confirms His ancient promises. While traditions vary on the total number, **Baptism** and **Communion** are universally recognized as the twin pillars of Christian ordinance.\n\n---\n\n### **Communion — The Table of Remembrance**\nThe Lord's Supper (Eucharist) acts as a perpetual monument in time to the substitutionary death of Jesus. It is more than a memory; it is a spiritual participation in the life of Christ.\n\n• It remembers the **broken body** of the Savior.\n• It seals the **New Covenant** in His blood.\n• It anticipates the future **Marriage Supper** of the Lamb.\n\n*Key Verses: Luke 22:19–20; 1 Corinthians 10:16–17; 1 Corinthians 11:23–26*\n\n---\n\n### **Circumcision — The Shadow of the Substance**\nCircumcision was the physical mark of Israel’s national and religious identity—a \"shadow\" of the reality to come. It marked the flesh to indicate that the seed of Abraham was set apart for God's global mission.\n\nHowever, the New Testament reveals that physical cutting was a signpost for **spiritual transformation**. In Christ, the physical requirement is superseded by the \"Circumcision of the Heart\"—a work of the Spirit that removes the stubbornness of the sinful nature.\n\n*Key Verses: Genesis 17:10–14; Romans 2:28–29; Colossians 2:11–12; Deuteronomy 10:16*\n\n---\n\n### **About the Author**\n**Written by Solat Nadeem**, Founder — Saul's Podship · Karachi, Pakistan.\nDedicated to equipping believers worldwide with historically grounded, theologically rigorous, and visually rich biblical scholarship.",
    "tables": [
      {
        "title": "I. VIEWS OF COMMUNION ACROSS DENOMINATIONS",
        "headers": [
          "Viewpoint",
          "📌 Meaning & 📖 Scripture",
          "💡 Theological Theme"
        ],
        "rows": [
          [
            "1",
            "📌 Transubstantiation\n📖 Matthew 26:26",
            "💡 The bread and wine literally become Christ’s body and blood. (Catholic/Orthodox Tradition)."
          ],
          [
            "2",
            "📌 Sacramental Union\n📖 1 Corinthians 10:16",
            "💡 Christ is \"in, with, and under\" the elements. (Lutheran Tradition)."
          ],
          [
            "3",
            "📌 Spiritual Presence\n📖 John 6:53–58",
            "💡 Christ is spiritually present to nourish the faith of the believer. (Reformed Tradition)."
          ],
          [
            "4",
            "📌 Memorial View\n📖 Luke 22:19",
            "💡 Symbolic remembrance only; \"Do this in remembrance of Me.\" (Baptist/Pentecostal Tradition)."
          ]
        ]
      },
      {
        "title": "II. COVENANT SIGNS: OLD VS. NEW",
        "headers": [
          "Aspect",
          "📌 Circumcision (Old)",
          "📌 Baptism (New)"
        ],
        "rows": [
          [
            "1",
            "Covenant: Abrahamic (Physical Lineage)",
            "Covenant: New (Spiritual Lineage)"
          ],
          [
            "2",
            "Subjects: Male Only",
            "Subjects: Men & Women (Gal 3:28)"
          ],
          [
            "3",
            "Age: 8 Days Old",
            "Age: Any (Entrance into Faith)"
          ],
          [
            "4",
            "Action: Cutting of Flesh",
            "Action: Immersion in Water"
          ],
          [
            "5",
            "Meaning: Physical Separation",
            "Meaning: Spiritual Resurrection"
          ]
        ]
      },
      {
        "title": "III. SACRAMENTAL TRADITIONS",
        "headers": [
          "Tradition",
          "📌 Number & 📖 Key Concept",
          "📝 Names of Sacraments"
        ],
        "rows": [
          [
            "Catholic",
            "📌 7 Sacraments\n💡 Means of Grace",
            "📝 Baptism, Eucharist, Confirmation, Reconciliation, Marriage, Holy Orders, Anointing."
          ],
          [
            "Protestant",
            "📌 2 Ordinances\n💡 Acts of Obedience",
            "📝 Baptism and the Lord's Supper (Communion)."
          ],
          [
            "Anglican",
            "📌 2 + 5\n💡 Dominical Sacraments",
            "📝 Primary: Baptism & Eucharist. Others: Confirmation, Marriage, etc."
          ]
        ]
      }
    ],
    "storyPanels": [
      {
        "id": "bap-1",
        "title": "The Jordan Baptism",
        "description": "Jesus descending into the waters as the heavens open and the Dove descends.",
        "scripture": "Matthew 3:16",
        "imagePrompt": "Cinematic wide shot of Jesus standing in the Jordan river, brilliant golden light beam from heavens, white dove descending, photorealistic, serene atmosphere.",
        "colorTheme": "#2196F3",
        "era": "Christ"
      },
      {
        "id": "sac-1",
        "title": "The Upper Room",
        "description": "The breaking of bread that changed history; the institution of the New Covenant.",
        "scripture": "Luke 22:19",
        "imagePrompt": "High-contrast painting of the Last Supper, focus on hands breaking bread, warm candlelight, dramatic shadows, Da Vinci inspired but more gritty and historical.",
        "colorTheme": "#8B1E3F",
        "era": "Christ"
      },
      {
        "id": "cir-1",
        "title": "Circumcision of the Heart",
        "description": "A metaphorical representation of the internal transformation from stone to flesh.",
        "scripture": "Ezekiel 36:26",
        "imagePrompt": "Conceptual art, a stone heart breaking open to reveal a living, glowing human heart inside. Ethereal light, surrealism, gold accents.",
        "colorTheme": "#D4AF37",
        "era": "Prophets"
      }
    ],
    "timeline": [],
    "scriptures": [],
    "interactiveMapPoints": [],
    "characterProfiles": [],
    "maps": []
  },
  "featured": false
};

export default volume_08;
